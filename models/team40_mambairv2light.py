...
def _ntuple(n):
    ...
    def parse(x):
        if isinstance(x, collections.abc.Iterable):
            return tuple(x)
        return tuple([x] * n) if isinstance(x, int) else x
...
class MambaIRv2Light(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        # self.student = MambaIRv2(**kwargs)
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        self.img_range = img_range
         # Load Teacher Model (Reference Only, No Training)
        self.teacher =None
        # if teacher_path:
        #     self.teacher = MambaIRv2(**kwargs)  # Initialize teacher model
        #     self.teacher.load_state_dict(torch.load(teacher_path, map_location='cpu'), strict=False)
    ...
    def forward(self, x):
        ...
        if self.teacher is not None:
            with torch.no_grad():  # Ensure teacher is only used as reference
                teacher_output = self.teacher(x)

    # Return both student and teacher outputs
        return (x, teacher_output) if teacher_output is not None else x
...
class MambaIRv2LightModel(MambaIRv2Light):
    """MambaIRv2 Light model for Efficient Super-Resolution."""

    def __init__(self):
        super(MambaIRv2LightModel, self).__init__()
    ...
    def load_weights(self, checkpoint_path):
        ...
        self.load_state_dict(checkpoint, strict=False)  # ✅ Change strict=False to allow flexibility

    def test(self):
        """Overlapping tile-based inference to reduce artifacts."""
        _, C, h, w = self.lq.size()
        tile_size = 200
        scale = 4
        overlap = 16  # ✅ Add overlap to prevent artifacts

        pad_h = (tile_size - h % tile_size) % tile_size
        pad_w = (tile_size - w % tile_size) % tile_size
        img = F.pad(self.lq, (0, pad_w, 0, pad_h), mode='reflect')

        _, _, H, W = img.size()
        stride = tile_size - overlap

        output = torch.zeros(1, C, H * scale, W * scale, device=self.lq.device)
        ...
        for i in range(0, H, stride):
            for j in range(0, W, stride):
                ...
                with torch.no_grad():
                    ...
                    tile_sr = self.net_g(tile)

                output[:, :, i*scale:i_end*scale, j*scale:j_end*scale] += tile_sr
                weight_map[:, :, i*scale:i_end*scale, j*scale:j_end*scale] += 1

        self.output = (output / weight_map)[:, :, :h * scale, :w * scale]  # ✅ Normalize overlapping regions
        # img_sr = util.tensor2uint(self.output)


    def test_selfensemble(self):
        """Apply self-ensemble testing (8-way TTA)."""
        def _transform(v, op):
        ...
        # Merge results using **simple averaging** instead of adding std()
        self.output = torch.stack(out_list, dim=0).mean(dim=0)  # ✅ Avoid adding unnecessary noise
