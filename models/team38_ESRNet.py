...
class AttentionBlock(nn.Module):
    def __init__(self,
        ...
        self.c1_r = FastConv(in_channels, mid_channels, gain1=2, s=1)
        self.c2_r = FastConv(mid_channels, mid_channels, gain1=2, s=1)
        self.c3_r = FastConv(mid_channels, out_channels, gain1=2, s=1)
...
class SimlifiedAttentionBlock(nn.Module):
    def __init__(self,
                 in_channels,
                 mid_channels=None,
                 out_channels=None,
                 bias=False):
        super(SimlifiedAttentionBlock, self).__init__()
        if mid_channels is None:
            mid_channels = in_channels
        if out_channels is None:
            out_channels = in_channels
...
class ESRNet(nn.Module):
    """
    Modifed from Swift Parameter-free Attention Network for Efficient Super-Resolution
    """
    ...
    def __init__(self,
        ...
        self.conv_1 = FastConv(in_channels, feature_channels, gain1=2, s=1)
        self.block_1 = AttentionBlock(feature_channels, bias=bias)
        self.block_2 = AttentionBlock(feature_channels, bias=bias)
        self.block_3 = AttentionBlock(feature_channels, bias=bias)
        self.block_4 = AttentionBlock(feature_channels, bias=bias)
        self.block_5 = AttentionBlock(feature_channels, bias=bias)
        self.block_6 = AttentionBlock(feature_channels, bias=bias)
        self.block_7 = SimlifiedAttentionBlock(feature_channels, bias=bias)
        
    
        self.conv_cat = conv_layer(feature_channels * 4, feature_channels, kernel_size=1, bias=True)
        self.conv_2 = FastConv(feature_channels, feature_channels, gain1=2, s=1)
    ...
    def forward(self, x):
        ...
        out_b1, out_b0_2  = self.block_1(out_feature)
        ...
        out_final = out_final + out_b7 * self.scale
...
if __name__ == "__main__":
    ...
    model = ESRNet(3, 3, upscale=4, feature_channels=26).to(device)
    model.eval()
