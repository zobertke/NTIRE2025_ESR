...
class ESA(nn.Module):
    def __init__(self, num_feat=24, conv=nn.Conv2d):
        ...
        self.conv_f_3 = Conv3XC(c_in=f, c_out=f)
        ...
        self.bias = nn.Parameter(torch.zeros(1, num_feat, 1, 1), requires_grad=True)
    ...
    def forward(self, input):
        ...
        cf_3 = self.conv_f_3(c1_)
        
        c4 = self.conv4((c3_1 + c3_2 + cf + cf_3))
        m = self.sigmoid(c4) - self.bias.expand_as(c4)
...
class SparseAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, bias, tlc_flag=True, tlc_kernel=96, activation='relu', input_resolution=None):
...
class SparseAttentionLayerBlock(nn.Module):
    def __init__(self, dim, restormer_num_heads=3, restormer_ffn_expansion_factor=2., tlc_flag=True, tlc_kernel=96,
        ...
        self.restormer_attn = SparseAttention(dim, num_heads=restormer_num_heads, bias=False, tlc_flag=tlc_flag,
                                              tlc_kernel=tlc_kernel, activation=activation,
...
def UpsampleOneStep(in_channels, out_channels, upscale_factor=4):
    ...
    """
    conv = nn.Conv2d(in_channels, out_channels * (upscale_factor ** 2), 1, 1, 0)
    pixel_shuffle = nn.PixelShuffle(upscale_factor)
    return nn.Sequential(*[conv, pixel_shuffle])


class HannahSR(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=24, num_block=4, num_out_ch=3, upscale=4,
                 rgb_mean=(0.4488, 0.4371, 0.4040), **kwargs):
        super(HannahSR, self).__init__()
        self.conv = BSConvU
        self.scale = upscale
        self.mean = torch.Tensor(rgb_mean).view(1, 3, 1, 1)

        # self.fea_conv = nn.Conv2d(num_in_ch, num_feat, 3, 1, 1)
        self.fea_conv = Conv3XC(c_in=num_in_ch, c_out=num_feat)
        ...
        self.res_1 = Conv3XC(c_in=num_in_ch, c_out=num_feat)
        self.res_2 = nn.Conv2d(num_feat, num_feat, 1, 1, 0)
    ...
    def forward(self, input):
        ...
        # out = self.upsampler(self.c2(self.GELU(self.c1(torch.cat([out_B1, out_B2, out_B3, out_B4], dim=1)))) + out_fea) + self.mean
        
        out_res = self.res_1(input)
        out_res = self.res_2(out_res)
        
        out_res = out_res + out_fea + out_B4
        out = self.upsampler(out_res) + self.mean
...
if __name__ == '__main__':
    ...
    model = AGDN_S(num_feat=24, upscale=4)
